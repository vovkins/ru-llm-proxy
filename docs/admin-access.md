...
В экспериментальном профиле интерфейс `codex-lb` и его API используют общий
порт `2455`. Он публикуется так же, как LiteLLM, но предназначен только для
внутренней сети. Клиенты обращаются к LiteLLM на `4000`; прямой доступ клиентов
к `codex-lb` не требуется. Первичная настройка и ротация служебного ключа:
