...
| Пароль администратора `codex-lb` | Администраторы пула подписок | Подписки, квоты, служебные ключи и настройки расширенного состава | Доступ разрешён только из доверенной внутренней сети; TOTP остаётся необязательным. |
...
В расширенном составе интерфейс `codex-lb` и его API доступны на порту `2455`
...
- Для состава с `codex-lb` отдельно сохраняются его PostgreSQL и том с ключом
