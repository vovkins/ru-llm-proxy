...
| Ключи внешних провайдеров | Только среда выполнения прокси | Вызовы Z.AI/OpenAI/Anthropic из прокси, который оплачивает и вызывает провайдера сам | Храните в менеджере секретов или защищённом `.env`; не кладите в настройки клиентов. |
| `LITELLM_MASTER_KEY` | Только экстренный доступ и административная автоматизация | Привилегированный административный API-ключ LiteLLM | Считайте секретом с полными правами. Храните в менеджере секретов, ограничивайте доступ CI, ротируйте при уходе оператора или подозрении на утечку. |
...
| Пароль администратора `codex-lb` | Администраторы пула подписок | Подписки, квоты, служебные ключи и настройки экспериментального профиля | Доступ разрешён только из доверенной внутренней сети; TOTP остаётся необязательным. |
| `CODEX_LB_API_KEY` | Только LiteLLM и менеджер секретов | Внутренний вызов `codex-lb` из LiteLLM | Не выдавайте пользователям и не используйте как ключ LiteLLM. Создавайте и ротируйте штатными средствами `codex-lb`. |
| OAuth-сессии ChatGPT | Только `codex-lb` и его зашифрованное хранилище | Вызовы OpenAI от имени добавленных подписок | Импортируйте из изолированных сессий; не помещайте `auth.json` в Git и не используйте импортированную копию одновременно в локальном Codex. |
...
- Размещайте административный интерфейс и административные API-маршруты как минимум за одной операторской границей:
  корпоративный SSO/OIDC/SAML, VPN, список разрешённых IP, mTLS, прокси нулевого доверия (zero-trust proxy) или
...
- Не допускайте попадания `LITELLM_MASTER_KEY` на рабочие станции, в клиентские
  настройки клиентов, снимки экрана, заявки и обычный вывод быстрых проверок.
...
В корпоративном профиле интерфейс `codex-lb` и его API доступны на порту `2455`
через тот же Nginx, который публикует LiteLLM на порту `80`. Отдельный Nginx и
прямая публикация панели из контейнера `codex-lb` не используются. Оба адреса
предназначены только для внутренней сети; клиентам доступ к панели не требуется.
Первичная настройка и ротация служебного ключа выполняются через
`make setup STACK=litellm-presidio-codex-lb`; пароль запрашивается без эха, а
ключ сохраняется в `.env` автоматически. Подробности:
...
SSO-группы, правила маршрутизации обратного прокси, частные административные сети, отдельный
...
| Оператор бюджетов и доступа к моделям | Административный интерфейс или контролируемый административный API-процесс | Менять бюджеты, лимиты частоты запросов, разрешённые модели и доступ команд | Изменения должны быть аудируемыми и привязанными к заявке. |
| Аудитор / только чтение | Роль только для чтения в административном интерфейсе, где доступно; иначе панели мониторинга и журналы | Смотреть использование, расходы, инвентаризацию ключей и административные действия | В OSS-установках давайте доступ только для чтения через панели или выгрузку журналов, а не через общие административные учётные данные. |
| Администратор экстренного доступа | Запечатанный секрет или привилегированная роль в менеджере секретов | Экстренное отключение, ротация административного ключа, восстановление после блокировки администратора | Требует регистрации инцидента, по возможности согласования двумя людьми и ротации после инцидента. |
...
- добавление, отключение и повторная авторизация OAuth-подписок `codex-lb`, а
  также выпуск его служебных ключей;
- ротация `LITELLM_MASTER_KEY`, `UI_PASSWORD`, ключей провайдеров или секретов базы данных;
- включение или отключение административного интерфейса, изменение SSO или политики обратного прокси;
...
- `make health STACK=<состав>` проходит;
...
- журналы доступа обратного прокси и SSO для `/ui` и административных API-маршрутов;
- журналы CI-задач для первичной автоматизации, с замаскированными секретами;
- GitOps или заявки для изменений конфигурации, моделей и провайдеров;
- записи резервного копирования/изменений базы данных для состояния ключей и пользователей LiteLLM.
- журналы административного интерфейса `codex-lb` и изменения его подписок,
  настроек и служебных ключей.
...
- метка времени;
- идентификатор заявки, изменения или инцидента, если он есть;
- результат: успех, ошибка или откат.
...
- `LITELLM_MASTER_KEY`, `LITELLM_SALT_KEY`, ключи провайдеров, `UI_PASSWORD` и
  пароль PostgreSQL являются сгенерированными секретами, а не тестовыми значениями.
...
- Для профиля `codex-lb` отдельно сохраняются его PostgreSQL и том с ключом
  шифрования; одна резервная копия базы LiteLLM их не заменяет.
