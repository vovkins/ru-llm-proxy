...
| --- | --- | --- |
| Nginx | `nginx` | Единая публичная точка входа (reverse proxy) на порт `80` → `litellm:4000`; большие тела запросов и потоковая передача (SSE). Подробнее о развёртывании в air-gapped среде: [air-gapped.md](air-gapped.md) |
