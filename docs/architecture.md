...
нормализации, токенизации, разбиения на окна, вывода модели, BIO-декодирования
или перевода смещений защёлкивает `failed`; текущие и последующие запросы
получают `503 required_ner_unavailable` до перезапуска.
...
жёсткому пределу. Перекрывающиеся результаты одного типа объединяются. Возможный
обрезанный фрагмент без полного результата приводит к ошибке, а не к скрытой
частичной детекции.
...
SHA-256 разрешённых файлов. Стадия `model-download` проверяет их при сборке;
рабочий образ использует только локальные файлы с `HF_HUB_OFFLINE=1`,
`TRANSFORMERS_OFFLINE=1`, `local_files_only=True` и `trust_remote_code=False`.

Цель `analyzer-test` наследует рабочие библиотеки, добавляет `pytest` из
`requirements-analyzer-test.txt`, но расположена до скачивания модели и не
увеличивается на вес NER. Рабочая цель `analyzer` не содержит `pytest`.
...
- Неполная сущность на границе окна приводит к безопасной ошибке.
