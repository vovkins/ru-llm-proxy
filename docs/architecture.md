...
с перекрытием 64. До четырёх окон выполняются одним пакетом BERT. Если остаются
только возможные обрезанные сущности, спорная граница один раз проверяется
окном, смещённым к центру. Неразрешимая неоднозначность приводит к безопасной
ошибке только текущего запроса.
