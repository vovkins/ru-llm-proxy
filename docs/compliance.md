...
| `admin-operator-boundary` | документация, статические проверки и подтверждения промышленного запуска | Клиентские, провайдерские и административные учётные данные разделены; административный интерфейс/API защищён SSO/reverse-proxy/частной сетью или отключён. |
...
включение `block` является решением конкретной установки. Публичные правила по умолчанию содержат только
