...
| Проверка с живым провайдером | `make guardrails-smoke STACK=<состав>`, `make test-e2e STACK=<состав>`, `make routing-smoke STACK=<состав>` | Реальные LiteLLM, защитные обработчики, протокол и маршрутизация работают вместе | Не доказывает отсутствие утечки: фактическая нагрузка внешнего провайдера не перехватывается |

Иными словами, успешная проверка с живым провайдером не доказывает отсутствие утечки.
Для экспериментального профиля `codex-lb` выполняйте тот же контур с
`CHAT_MODEL=gpt-5.6-luna` и `RESPONSES_MODEL=gpt-5.6-luna`, а также проверяйте
состояние обеих OAuth-подписок. Полный порядок: [codex-lb.md](codex-lb.md).
...
В профиле `codex-lb` пароль его администратора, OAuth-сессии и
`CODEX_LB_API_KEY` образуют отдельную административную границу. Клиентам они не
выдаются; интерфейс `codex-lb` доступен только из доверенной внутренней сети.
...
- в профиле `codex-lb` LiteLLM обращается к нему по внутренней сети, а внешний
  доступ к OpenAI имеет только `codex-lb`;
- Analyzer, Redis и обе базы PostgreSQL не имеют интернет-доступа во время работы;
