...
В зависимости от LiteLLM/FastAPI wrapper JSON может быть обёрнут как `detail.error` или `error.provider_specific_fields.error`, но поля `message`, `type`, `code`, `details.categories` и `details.rules` остаются обязательными.
