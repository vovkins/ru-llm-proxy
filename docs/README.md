...
| Маршрутизация провайдеров моделей и закрепление маршрута за ключом | [routing.md](routing.md) |
...
| Deployment | Внутренняя запись провайдера модели в LiteLLM. В обычном тексте пишем «провайдер модели»; английское слово оставляем только при ссылке на имя поля или метрики LiteLLM. |
...
| --- | --- |
| Промышленный запуск в Kubernetes | Проектируется в #59, чтобы не смешивать документационный рефакторинг и инфраструктурный дизайн. |
