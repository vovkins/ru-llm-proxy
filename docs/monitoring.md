...
| Redis | `redis-cli ping` | Доступность temporary mapping store |
...
6. Выполнить PII smoke request и проверить `make monitor-smoke`.
7. Если есть regression, откатить image tag/digest в `docker-compose.yml` и пересоздать `litellm`.
