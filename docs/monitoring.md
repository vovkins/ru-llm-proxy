...
- живы ли LiteLLM, Analyzer, Redis и PostgreSQL, а в расширенном составе
...
смотрите через `docker compose ps`; для расширенного состава отдельно
проверьте `curl -fsS http://localhost:2455/health/ready`. В изолированной
корпоративной конфигурации
...
Недоступность HTTP-соединения с Analyzer и хранилища таблиц замен также
возвращается клиенту как временная ошибка `503` с `Retry-After: 1`. В режиме
`fail_closed` запрос не передаётся провайдеру без завершённого анализа и
сохранённой таблицы замен.
...
- `codex-lb`: `http://localhost:9090/metrics` в расширенном составе.
...
Последнюю задачу сбора добавляйте только при запуске состава с `codex-lb`.
...
При жёсткой остановке одной реплики LiteLLM выполнявшийся на ней запрос может
получить HTTP 502, хотя остальные реплики продолжают работу. Недоступность Redis
в безопасном режиме даёт HTTP 503 с `Retry-After`; недоступность PostgreSQL
может вернуться платформенным `503 no_db_connection` без этого заголовка.
Отслеживайте коды отдельно от общей доли 5xx и согласуйте повтор POST-запроса с
семантикой клиента, чтобы не дублировать уже начатую операцию у провайдера.
