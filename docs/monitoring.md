...
API-ключи, OAuth-токены и непрозрачное состояние Responses API из
...
нужно. Для временного образа с несколькими ChatGPT/Codex OAuth-профилями сначала
выполните [регламент обновления тонкого ответвления](litellm-fork-update.md).
Команда ниже только скачивает уже указанную в Compose версию:
