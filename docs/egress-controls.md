...
В ветке `air-gapped-environment` загрузки при сборке и вызовы провайдеров из
LiteLLM проходят через корпоративный прокси. Корпоративный CA добавляется в
хранилища доверия образов; проверка TLS не отключается. Analyzer после сборки
остается без внешних соединений. Настройки и порядок проверки описаны в
