...
- в расширенном составе `codex-lb` может ходить только к своей базе и
...
В изолированной корпоративной среде загрузки при сборке и внешние
вызовы проходят через корпоративный прокси. Корпоративный CA добавляется в
хранилища доверия образов; проверка TLS не отключается. В расширенном составе
...
Для `docker-compose.codex-lb.yml` добавляются направления:
...
модели LiteLLM. Полный порядок эксплуатации состава: [codex-lb.md](codex-lb.md).
...
6. Подтвердите, что `presidio-analyzer`, `redis` и `db` не могут обращаться к произвольным интернет-хостам.
   Для состава с `codex-lb` выполните ту же проверку для `codex-lb-db`, а из
...
- В составе с `codex-lb` отдельно контролируйте внешние соединения `codex-lb` и
