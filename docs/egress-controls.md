...
В изолированной сети выход к провайдерам и загрузки при сборке идут через корпоративный перехватывающий (re-signing) HTTP-прокси, который переподписывает TLS. Проект доверяет корневому CA этого прокси, а не отключает проверку TLS.

- **Сборка образов (build-time egress).** `apt`, внутреннее PyPI-зеркало (`PIP_INDEX_URL` / `PIP_TRUSTED_HOST` / `PIP_PROXY`), spaCy `ru_core_news_sm` и архив DeepPavlov (`DEEPPAVLOV_NER_MODEL_URL`) тянутся через прокси, а `certs/proxy-ca.crt` запекается в trust store образа. Прокси передаётся build-аргументами, потому что `docker build` не наследует `environment:` сервиса.
- **Работа (runtime egress).** `litellm` обращается к разрешённым FQDN провайдеров через прокси; доверие CA — безопасная альтернатива `SSL_VERIFY=False`. `presidio-analyzer` после сборки не ходит в интернет для обработки запросов (кроме единичной первой загрузки весов HuggingFace, которая тоже верифицируется по прокси-CA и кэшируется).
- **Единая точка входа.** Клиенты обращаются к `nginx` (порт `NGINX_HTTP_PORT`, по умолчанию `80`), который проксирует на `litellm:4000`. При настройке firewall/CNI добавьте `nginx` во входящий список, а корпоративный прокси-хост — в разрешённый исходящий для `litellm`.

Хост зеркала и прокси должны быть согласованы с `NO_PROXY`: внутренние сервисы и домены идут в обход прокси, а хост PyPI-зеркала — либо через выделенный `PIP_PROXY`, либо напрямую (тогда добавьте его в `NO_PROXY`). Переменные air-gapped среды описаны в [configuration.md](configuration.md), пошаговое развёртывание — в [air-gapped.md](air-gapped.md).
