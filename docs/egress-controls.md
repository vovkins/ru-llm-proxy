...
В ветках `air-gapped-environment` и
`air-gapped-environment-codex-lb-integration` загрузки при сборке и внешние
вызовы проходят через корпоративный прокси. Корпоративный CA добавляется в
хранилища доверия образов; проверка TLS не отключается. В объединённом профиле
это относится и к `codex-lb`. Analyzer после сборки остаётся без внешних
соединений. Настройки и порядок проверки описаны в [air-gapped.md](air-gapped.md).
