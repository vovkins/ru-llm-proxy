...
Расширенный состав подключает штатный `codex-lb` к неизменённому LiteLLM.
...
Состав не меняет обычный запуск с GLM и включается только через
`docker-compose.codex-lb.yml`. Сейчас закреплён стабильный официальный образ
`v1.24.0`; локальных патчей LiteLLM или `codex-lb` нет. Производный образ
добавляет только корпоративную цепочку доверия TLS, без изменения кода
платформы.
...
Для изолированной корпоративной среды предварительно поместите сертификаты в
`certs/*.crt` и заполните `HTTP_PROXY`, `HTTPS_PROXY` и `NO_PROXY` по
[соответствующей инструкции](air-gapped.md).
...
- TOTP не обязателен в этом составе, но может быть включён администратором
...
не публикуются. Nginx работает по HTTP; завершение HTTPS остаётся
...
- [Выпуск codex-lb 1.24.0](https://github.com/Soju06/codex-lb/releases/tag/v1.24.0)
