...
каждой подписке и отсутствие секретов в журнале. На обеих подписках отдельно
проверены `gpt-5.6-sol`, `gpt-5.6-terra` и `gpt-5.6-luna`; общий алиас
`gpt-5.6` возвращал HTTP 400 и поэтому не входит в конфигурацию. Через Luna
проверены обычное и потоковое продолжение Responses API с
`reasoning.encrypted_content`, маскирование нового открытого текста и продолжение
между двумя OAuth-сессиями.
