...
- калибровка recognizer thresholds и расширение address corpus: issue #15 / PR #23;
- переиспользование Redis/httpx clients и pool limits внутри guardrail: issue #17 / PR #22.
...
Текущий `main` использует `score_threshold=0.35`: bare ИНН без контекстных слов может не пройти порог до слияния #15/#23, а `RU_ADDRESS` остаётся regex-based покрытием базовых российских адресных форматов.
